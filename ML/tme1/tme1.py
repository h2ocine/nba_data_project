...
class Histogramme(Density):
    ...
    def fit(self,x):
        ...
        self.histo =  np.histogramdd(x,bins=self.steps, density = True)
    ...
    #A compléter : retourne la densité associée à chaque point de x
        def to_bin(x):
            ...
            """ 
            indices = ((x - self.min_values) / (self.max_values - self.min_values) * self.steps).astype(int)
            return indices
            
        # Obtention des bornes min et max de chaque dimension de l'histogramme
        histogram, bin_edges = self.histo
        
        # Calcul des indices correspondant aux coordonnées du point x
        indices = to_bin(x)
        
        # Récupération de la valeur de densité dans l'histogramme
        density_value = self.histo[tuple(indices)]  # Pour un histogramme en 2D
