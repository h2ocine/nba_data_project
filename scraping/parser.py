...
def parse_teams(url : str, title : str, parse_players = False) -> pd.DataFrame:
    ...
    if(parse_players):
        teams = [tag.get_text() for tag in names_table_tag[15:-27]]
    else:
        teams = [tag.get_text() for tag in names_table_tag[16:-27][::2]]
    ...
    for i in range(len(teams)):
        ...
        attributes = [str(rank),teams[i]] # Initialise with the team name and its rank
        for td in td_tags:
            attribute = td.get_text()
            attributes.append(attribute)
        data.append(attributes)
...
def parse_players(url: str, title: str) -> pd.DataFrame:
    ...
    names_table_tag = soup.find_all('a')
    players = [tag.get_text() for tag in names_table_tag[15:-26]]
